...
This script uses the nsys command-line tool to check the system's profiling environment. The output shows that various components, such as the timestamp counter, CPU profiling, and last branch record support, are available on the system. Additionally, the Linux kernel version and paranoid level are also checked and reported. Finally, a link to the product documentation is provided for more information on how to configure the profiling environment.
